...
>
> 当前批量审查设计：`batch-review/docs/13-skill-batch-security-review-and-scoring-design.md`
>
> 当前批量审查任务：`batch-review/docs/14-skill-batch-review-implementation-tasks.md`
>
> 当前批量审查使用说明：`batch-review/docs/15-skill-batch-review-script-user-guide.md`
>
> 当前批量审查快速入口：`batch-review/docs/16-skill-batch-review-quick-start.md`
>
> 当前逐 Skill 需求：`batch-review/docs/17-per-skill-review-requirements.md`
>
> 当前逐 Skill 设计：`batch-review/docs/18-per-skill-review-design.md`
>
> 当前逐 Skill 任务：`batch-review/docs/19-per-skill-review-implementation-tasks.md`
>
> 当前初始化与免参数操作说明：`batch-review/docs/20-project-initialization-and-guided-operation.md`
>
> 当前 Windows AI 客户端执行说明：`batch-review/docs/21-windows-ai-client-batch-review-guide.md`
...
- CSV 采用“必需字段校验、扩展字段透传”：新增产品线、部门、用户或其他业务列不得导致导入失败，所有扩展列原样保留到结果 CSV；
...
- AI 审查规则统一位于 `batch-review/skills/skill-security-review/`，由 Codex CLI 或 Claude Code 调用公司内网模型执行；两端的 `batch-review/.agents/`、`batch-review/.codex/`、`batch-review/.claude/` 只负责发现和隔离调度。每个 Skill 使用独立上下文，AI 只读取 Skill Package 和最小任务元数据，不读取静态扫描报告、Manifest 或历史结果，不联网、不执行被审查内容；静态结果由程序独立合并；该入口参考 UseAI-pro 的 `skill-vetter` 与 `skill-auditor`，但不是上游副本；
...
- 批次完成后由脚本生成单文件 HTML 审查工作台，支持总览、单 Skill、问题、仓库和提交人
  多层视图以及统一筛选和导出；HTML 只嵌入脱敏派生数据和受限证据完整性索引。
...
11. 完成仓库文件修改并验证后，默认在同一轮自动提交并通过 SSH 推送；只有用户明确要求不提交或不推送时才保留本地改动。
...
- `batch-review/docs/13-skill-batch-security-review-and-scoring-design.md`：存量 Skill 批量安全审查、质量评分与私密候选归档设计
- `batch-review/docs/14-skill-batch-review-implementation-tasks.md`：批量审查 T00–T53 实施任务、依赖和验收标准
- `batch-review/docs/15-skill-batch-review-script-user-guide.md`：批量审查脚本配置、逐仓库执行、AI 结果导入、报告、清理和排障说明
- `batch-review/docs/16-skill-batch-review-quick-start.md`：公司配置模板、一键启动器与 Claude Code Skill 触发说明
- `batch-review/docs/17-per-skill-review-requirements.md`：逐 Skill 下载、归档、结果和筛选需求
- `batch-review/docs/18-per-skill-review-design.md`：仓库级一次归档、逐 Skill 目录、复用和 JSON/CSV 设计
- `batch-review/docs/19-per-skill-review-implementation-tasks.md`：逐 Skill 审查实施任务与验收场景
- `batch-review/docs/20-project-initialization-and-guided-operation.md`：首次初始化、状态判断、双击入口和 ask-cc 使用说明
- `batch-review/docs/21-windows-ai-client-batch-review-guide.md`：Windows 下使用 Codex CLI 或 Claude Code 的统一批量审查入口、隔离边界和环境检查
- `batch-review/AGENTS.md`：批量安全审查工作区边界、脚本优先原则和 AI 队列调度规则
- `batch-review/`：存量 Skill 批量审查程序、脱敏配置样例和本地测试
- `batch-review/skills/skill-security-review/`：Codex CLI 与 Claude Code 共用的只读 AI 安全与质量审查规则（参考 `skill-vetter`/`skill-auditor`，非上游副本）
- `batch-review/.agents/skills/`、`batch-review/.codex/agents/`：Codex CLI 的项目级 Skill 和隔离审查 Agent 入口
- `batch-review/.claude/skills/`、`batch-review/.claude/agents/`：Claude Code 的项目级 Skill 和隔离审查 Agent 入口
- `batch-review/.claude/skills/ask-cc/`：只读分析当前项目状态并给出唯一下一步的 Claude Code 项目级入口
- `batch-review/.claude/skills/auto-skill-review/`：完成当前及后续 AI 审查并自动推进 Skill、仓库和批次状态的 Claude Code 项目级入口
- `batch-review/.agents/skills/auto-skill-review/`：完成当前及后续 AI 审查并自动推进 Skill、仓库和批次状态的 Codex CLI 项目级入口
