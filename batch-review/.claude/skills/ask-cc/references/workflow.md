...
|---|---|---|
| `INITIALIZE` | 尚无本机配置和操作状态 | Windows 双击 `init.cmd`；Linux/CentOS/macOS 执行 `./init.sh` |
| `EDIT_CONFIG` | 配置缺失、示例值未替换或 CSV 不可用 | 打开状态输出中的唯一 `config_path`，修正列出的问题 |
| `INSTALL_SCANNERS` | 配置正常，但固定版本扫描器不存在 | 双击或执行 `review.cmd`/`./review.sh` 并确认安装 |
...
| `VIEW_RESULTS` | 批次完成 | 展示状态输出中的 CSV/JSON/HTML 结果路径 |
