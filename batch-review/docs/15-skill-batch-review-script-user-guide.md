...
`run.sh` 和 `run.cmd` 使用仓库/Skill 两级启动器。CSV 中每个纳入行必须有唯一
...
本文面向实际执行批量审查的 CM、安全人员和平台维护人员，说明如何使用本项目完成以下工作：
...
`run.sh` 与 `run.cmd` 负责记住仓库和 Skill 进度。在 Codex CLI 中调用
...
`packages/skillspector-2.5.1-py3-none-any.whl`，并在安装前固定校验 SHA-256
...
```bash
python3.12 tools/install_scanners.py \
...
```powershell
py -3.14 tools/install_scanners.py `
...
依赖仍只从公司源下载。所有受支持环境使用 NVIDIA v2.5.1 官方声明的直接运行依赖范围，并由公司源
选择其中实际存在的兼容版本。默认强制只安装 wheel。Windows 下 Cisco 的依赖链
...
在部分 Windows 环境中，SkillSpector 的 Typer 命令可能完成扫描却没有创建 `--output` 指定的文件。
安装器会自动再用其官方 JSON 标准输出模式验证一次；只有标准输出能解析为完整 JSON 时才通过，普通日志、
空输出或无效 JSON 仍会失败，并在错误中附带扫描器输出用于排障。该回退只用于安装冒烟，不改变正式扫描
报告的受限证据保存规则。
...
并对 `WinError 5` 做短暂重试；不会扩大清理范围，仍只删除当前批次中经过路径校验的单个 Skill
临时下载目录。遇到旧版本的“拒绝访问”后，拉取新版直接再次双击 `review.cmd`，不需要手工进入
`.transport.git` 删除文件。若旧版本留下的目录正好属于当前同一个 Skill 任务，新版会先自动清理
该唯一残留再重新下载；其他任务或无法确认归属的目录仍不会自动删除。
...
```bash
cp config/review.example.toml /secure/path/review.toml
```
...
```toml
[ai]
skill_path = "/path/to/batch-review/skills/skill-security-review"
result_schema_path = "/path/to/batch-review/skills/skill-security-review/references/review-result.schema.json"
```
