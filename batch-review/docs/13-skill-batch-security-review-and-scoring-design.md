...
本文件说明存量 Skill 批量安全审查、质量评分和候选归档的执行规则。它不涉及 SkillHub
平台上架，也不改变由产品线确认和上传的责任边界。配套脚本、客户端 Skill 和执行说明均
包含在本项目目录内。
...
下表只定义流程需要识别的字段。CSV 可以增加产品线、部门、负责人或其他扩展列；只要必需字段存在且表头不重复，扩展列不会导致导入失败，并会原样保留到审查结果表。
...
系统先用 Skill Root 的目录名称查找同名候选，再比较已经在快照阶段生成的
`skill_digest`。该比较不需要重新逐文件读取目录，适合数百个 Skill 的批量处理。

`skill_digest` 比较相对路径、文件类型、权限、符号链接目标和文件原始内容，不比较文件
时间戳。名称相同但 Digest 不同，说明实际内容不同，必须正常执行两套静态扫描和 AI
审查；名称不同的 Skill 不自动复用，即使内容碰巧相同也不自动合并。

同名且 `skill_digest` 相同，可以复用已经安全审查通过的结果，但必须同时满足：
...
每次复用都生成一条 `RESULT_REUSED` 记录，保留当前来源 Revision、原批次、原任务和原始
证据引用，并在批次明细和 HTML 报告中说明复用原因。任何版本、策略或证据条件不满足时，
自动回到完整审查，不能只复制“通过”。
...
Claude Code 通过项目 `.claude/skills/` 的轻量适配入口发现该 Skill，Codex CLI 通过
`.agents/skills/` 发现同一规则。批次控制层必须在两套静态报告完成后显式调度单项审查，
不能依赖模型自行判断触发时机。每个 Skill 使用独立上下文，只读取 Skill Package、结果
Schema 和最小任务元数据；不向模型提供静态扫描报告、Manifest 或历史结果。审查 Agent
只开放读取能力，以及仅写入指定结果 JSON 的能力，并禁用 MCP 和其他执行类工具。
