...
3. 从对应模板生成 `config/review.local.toml`；
...
Windows 的 `review.cmd`、Linux/CentOS/macOS 的 `./review.sh` 仍可作为
手动排障入口。
...
| 内容结果可复用 | 自动记录复用关系并继续 |
| 批次完成 | 显示结果 CSV、JSON 和 HTML 的绝对路径 |
...
该 Skill 会先调用本项目受信脚本完成计划、下载和静态扫描，再读取当前仓库 AI 队列，
...
```text
config/review.local.toml
    当前机器实际使用的配置

.batch-review/operator-state.json
    当前配置路径和批次号
