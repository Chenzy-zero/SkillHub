...
class ConfigTests(unittest.TestCase):
    ...
    def test_custom_client_policy_path_is_not_redirected(self) -> None:
        base = Path(self.tempdir.name)
        custom = base / ".claude/skills/skill-security-review"
        (custom / "references").mkdir(parents=True)
        (custom / "SKILL.md").write_text("---\nname: custom-review\n---\n", encoding="utf-8")
        custom_schema = custom / "references/review-result.schema.json"
        custom_schema.write_text("{}\n", encoding="utf-8")

        config = load_config(self.write_config())

        self.assertEqual(config.ai.skill_path, custom.resolve())
        self.assertEqual(config.ai.result_schema_path, custom_schema.resolve())
