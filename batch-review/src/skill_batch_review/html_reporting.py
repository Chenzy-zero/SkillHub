    ...
    *,
    ...
    for index, row in enumerate(details, 1):
        ...
        rows_html.append(
            ...
            f'<td><strong>{_escape(row["skill_name"])} {(_badge("结果复用") if row.get("reuse_status") == "RESULT_REUSED" else "")}</strong><small>{_escape(row["normalized_skill_path"])}</small></td>'
        ...
        reuse_detail = ""
        if row.get("reuse_status") == "RESULT_REUSED":
            reuse_detail = (
                f'<div><dt>结果复用</dt><dd>{_badge("RESULT_REUSED")} '
                f'{_escape(row.get("reuse_reason") or "同名 Skill Root 内容完全一致")}<br>'
                f'<small>来源批次 {_escape(row.get("reused_from_batch_id") or "未记录")} · '
                f'来源任务 {_escape(row.get("reused_from_task_id") or "未记录")} · '
                f'{_escape(row.get("content_compare_method") or "SHA-256")} · 忽略时间戳</small></dd></div>'
            )
