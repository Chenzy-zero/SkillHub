...
| S13 | 单文件 HTML 报告 | 总览、Skill、问题、仓库、提交人与证据链 | 联动筛选；按当前视图导出 CSV；筛选结果导出 JSON；不嵌入原始证据或外部附件 |

当前状态：S01–S13 已完成本地实现和模拟验证；S03 使用正式 Gerrit 已验证可用的整仓无历史
`git-upload-archive`。S13 由批次收口脚本自动生成 HTML，正式数据源仍是单项结果和批次 JSON。
...
9. HTML 在断网环境中独立打开，按仓库、产品线、提交人、结论和问题等级筛选后，各层级
   数量同步变化；当前视图 CSV 和筛选结果 JSON 可导出；问题可追溯到来源版本和证据索引。
