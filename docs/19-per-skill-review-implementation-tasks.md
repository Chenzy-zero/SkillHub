...
| S03 | 单 Skill partial fetch | 临时裸仓库和固定分支获取 | 服务端忽略 filter 时失败；不回退完整 clone |
...
`--filter=blob:none` 能力。HTML 视觉与筛选导出不在本轮实现范围，正式数据源已固定为批次
JSON。
...
5. Gerrit 不支持 partial clone filter 时停止，且不留下伪装为纯 Skill 的结果；
