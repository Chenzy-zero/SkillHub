...
批次完成时从单项持久化结果和受限证据区生成：

```text
results/<batch_id>/skill-security-review-report.html
```

报告不依赖 CDN、字体、脚本或图片附件，CSS、JavaScript 与脱敏数据全部内嵌，断网后可直接
打开。页面以 1920×1080 的企业审计工作台为主要设计画布，同时为较窄窗口提供响应式布局。

报告包含五个共享筛选范围的层级：

1. 总览：当前 Skill、仓库、提交人、高危问题、安全通过和结果复用数量；
2. 单 Skill：来源、双静态工具、AI、综合结论、质量和问题数；
3. 问题清单：严重等级、问题、扫描来源、Skill、仓库和文件定位；
4. 仓库：Skill 数、问题数、Critical/High 数、提交人数和平均质量；
5. 提交人：按 `user_name + user_email` 汇总仓库、Skill 和问题。

仓库、产品线、提交人、安全结论、问题等级和全文检索共同作用于全部视图。当前视图可导出
Excel 兼容的 UTF-8 BOM CSV；当前筛选范围可导出结构化 JSON。CSV 写出前对以 `= + - @`
开头的单元格做公式注入保护。

单 Skill 详情用一条证据链表示：

```text
来源锁定 → Cisco → SkillSpector → AI 审查 → 综合判定
```

问题详情同时展示仓库、分支、Skill 路径、问题文件和行号、Revision、内容 SHA-256、问题
指纹、扫描来源引用、脱敏证据摘录和处理建议。原始扫描器输出、AI 导入结果和包清单仍留在
`evidence_root`；HTML 仅保存相对路径、文件类型、大小和 SHA-256 完整性索引，不复制原文，
也不嵌入工作站绝对路径。所有来自 Skill 或扫描器的文本通过 JSON 安全编码并使用 DOM
`textContent` 渲染，防止被审查内容在打开报告时形成脚本注入。
