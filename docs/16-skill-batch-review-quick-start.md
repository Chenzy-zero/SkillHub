...
把本目录作为项目根目录打开。Windows 首次双击 `init.cmd`，Linux/CentOS/macOS 首次执行
`./init.sh`。

初始化入口会创建真实的本机配置 `config/review.local.toml`；该文件被 Git 忽略，
...
```bash
cp config/review.company.example.toml \
  config/review.company.toml
```

使用推荐初始化入口时无需手工执行上述复制命令，程序会自动创建
`config/review.local.toml` 并告诉执行人员还需要填写哪些字段。
...
```bash
./run.sh plan \
  --config config/review.company.toml \
...
```text
run.cmd plan --config config\review.company.toml --batch-id baseline-20260901
```
...
在本项目根目录启动公司批准的 Codex CLI 或 Claude Code。输入对应触发指令：
...
自动 Skill 会重复执行上述动作，直至批次完成，并在批次结束时生成单项 JSON、批次
CSV/JSON 和可筛选、可导出的单文件 HTML 审查报告。已完成的旧批次若缺少 HTML，可再次
运行统一入口补生成报告，不会重新下载或扫描 Skill。
...
Windows 将 `./run.sh` 替换为 `run.cmd`。入口会依次查找 Python
...
```bash
python3.12 tools/discover_git_skills.py \
  --repository . \
...
- AI 审查规则和结果 Schema 的路径；默认统一位于 `skills/skill-security-review/`。
