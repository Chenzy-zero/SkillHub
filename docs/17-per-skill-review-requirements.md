...
6. 对永久 Skill 文件夹逐一执行两套静态检查；当前仓库静态结果齐全后生成 AI 队列，由独立 Agent 批量审查；
...
- JSON：保留结构化问题和追溯关系，作为 HTML 与后续汇总的正式数据源；
- HTML：单文件离线报告，同时提供总览、单 Skill、问题、仓库和提交人视图。

HTML 使用同一套筛选条件联动所有视图，支持按仓库、产品线、`user_name + user_email`、
安全结论、问题等级和全文关键词筛选。CSV 导出随当前视图切换为 Skill、问题、仓库或
提交人明细，JSON 导出保留当前筛选范围内的完整结构化结果。单 Skill 和单问题详情必须
显示仓库、分支、路径、Revision、内容 SHA-256、扫描来源、规则、文件行号和证据索引。
原始扫描器内容不嵌入 HTML，只展示脱敏证据摘录和受限证据文件的相对路径、大小与
SHA-256，避免分发报告扩大敏感数据范围。
