...
def generate_report(db_path, report_dir, logger=None):
    ...
    source_rows = "".join(
        "<tr><td>{}</td><td>{}</td><td><code>{}</code></td><td>{}</td><td>{}</td><td><code>{}</code></td><td><code>{}</code></td></tr>".format(
            esc(row["skill_name"]), esc(row["repository"]), esc(row["skill_path"]), esc(row["status"]),
            row["revision_count"], esc(short(row["latest_revision"])), esc(short(row["latest_digest"])),
        )
        for row in sources
    ...
    change_rows = "".join(
        "<tr><td>{}</td><td>{}</td><td>{}</td><td>{}</td><td>{}</td><td>{}</td><td>{}</td></tr>".format(
            esc(row["change_number"]), esc(row["patchset"]), esc(row["repository"]), esc(row["branch"]),
            esc(row["subject"]), row["affected_count"], esc(row["observed_at"]),
        )
        for row in changes
    ) or '<tr><td colspan="7">暂无数据</td></tr>'
    ...
    event_rows = "".join(
        "<tr><td>{}/{}</td><td>{}</td><td>{}</td><td><code>{}</code></td><td>{}</td></tr>".format(
            esc(row["change_number"]), esc(row["patchset"]), esc(row["repository"]), esc(row["action"]),
            esc(row["source_key"] or ""), esc(row["trigger_file"] or ""),
        )
        for row in events
...
</div></body></html>""".format(cards=card_html, sources=source_rows, changes=change_rows, events=event_rows)
